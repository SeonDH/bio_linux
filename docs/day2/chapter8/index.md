...
| `Ctrl + f` | 화면의 한 페이지 아래로 이동 (forward)|
...
- `Ctrl + u`를 사용할 때 `u` 명령어와 혼동하지 않도록 주의해야 한다.
...
> `p`, `P`는 복사한 내용의 단위에 따라 동작이 달라진다. `yy`처럼 행 전체를 복사하면 `p`는 현재 행 아래, `P`는 현재 행 위에 붙여넣는다. `yw`처럼 단어 일부를 복사하거나 비주얼 모드에서 문자 단위로 복사하면 `p`는 커서 뒤, `P`는 커서 앞에 붙여넣는다.
...
| `/문자열` /`?문자열`     | 아래 / 위 방향 검색    |
| `n` / `N`           | 다음 / 이전 결과 이동   |
| `:10,30g/foo/`      | 10행부터 30행 사이에서 `foo`가 포함된 행 확인 |
| `:s/old/new`        | 현재 행 첫 번째 일치 치환 |
| `:s/old/new/g`      | 현재 행 모든 일치 치환 |
| `:<범위>s/old/new`   | 범위 내 각 행의 첫 번째 일치 치환 |
| `:%s/old/new/g`     | 전체 치환           |
