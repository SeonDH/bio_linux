...
더 자세한 설명은 [번외: 컴퓨터의 구성 요소와 동작 원리](extra/computer_components.md)를 참고한다.
...
- **응용 프로그램**: 사용자가 직접 실행해서 쓰는 프로그램이다. 예를 들면 개발 도구, 편집기, 네트워크 유틸리티 등이 있다.
- **셸(Shell)**: 사용자가 명령어를 입력하는 인터페이스다. Bash, Zsh 같은 셸이 여기에 해당하며, 사용자의 명령을 해석해 커널에 전달한다.
- **커널(Kernel)**: 하드웨어와 소프트웨어를 연결하는 운영체제의 핵심이다. 메모리, 파일 시스템, 프로세스, 장치 등을 실제로 관리한다.
...
### 관계
- 사용자는 셸에 명령을 입력한다.
- 셸은 그 명령을 해석해서 커널에 전달한다.
- 커널은 실제로 시스템 자원과 하드웨어를 조작한다.
- 응용 프로그램은 이런 과정을 사용자가 덜 의식하도록 도와주는 도구다.

더 자세한 설명은 [번외: 응용 프로그램, 셸, 커널의 관계](extra/app_shell_kernel.md)를 참고한다.
