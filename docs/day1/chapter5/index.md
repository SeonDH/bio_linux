...
- 리눅스 파일은 다음 세 가지 요소로 구성된다:
  ...
  - **inode**: 파일의 메타데이터와 데이터 블록 위치 정보를 담는 구조
...
- inode는 파일의 실제 내용을 저장하지 않는다.
- 대신 파일의 속성 정보와 실제 데이터 블록을 가리키는 포인터를 저장한다.
- 각 파일은 고유한 inode 번호를 갖고, 이 번호로 파일의 메타데이터를 구분한다.
...
- `ls -i`를 사용하면 맨 앞에 출력되는 값이 inode 번호이다.
...
- 원본 삭제 후에도 하드링크가 존재하면 데이터는 유지됨
  - inode의 링크 카운트가 0이 되면 해당 inode와 데이터 블록이 해제된다.
