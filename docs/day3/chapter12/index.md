...
| 디스크 | 파일 저장 공간과 읽기/쓰기 | 파일 읽기/쓰기 지연, 용량 부족 오류 | `df -h`, `du -h`, `iostat -x 1` |
...
`top` 상단의 `%Cpu(s)` 줄에서 `wa` 값을 볼 수 있다. `wa`는 I/O wait으로, CPU가 디스크나 네트워크 I/O를 기다린 시간의 비율이다.

```text
%Cpu(s):  5.0 us,  2.0 sy, 80.0 id, 13.0 wa
```

- `wa`가 계속 높으면 디스크 I/O가 느려서 작업이 기다리는 상황일 수 있다.
- `top`은 디스크 I/O 자체를 자세히 보여주지는 않으므로, 디스크 상태는 `iostat -x 1`로 추가 확인한다.
...
`top`에서 CPU 사용률과 메모리 사용량은 낮은데 작업이 느리고, `wa` 값이 계속 높다면 디스크 I/O 병목을 의심할 수 있다. 이때는 디스크 용량 문제인지, 실제 읽기/쓰기 병목인지 나누어 확인한다.
...
- `df -h`: 디스크 파티션의 전체 사용량을 확인한다.
- `du -h --max-depth=1`: 현재 디렉터리 아래에서 어떤 폴더가 큰지 확인한다.
- `iostat -x 1`: 디스크 읽기/쓰기 상태를 1초마다 확인한다.

`iostat`만 실행해도 디스크 정보가 나오지만, 초보자가 병목을 판단하기에는 `iostat -x 1`이 더 좋다.

주로 보는 항목은 다음과 같다.

| 항목 | 의미 | 해석 |
| --- | --- | --- |
| `r/s` | 초당 읽기 요청 수 | 읽기 작업이 얼마나 많은지 확인 |
| `w/s` | 초당 쓰기 요청 수 | 쓰기 작업이 얼마나 많은지 확인 |
| `rkB/s` | 초당 읽은 데이터 양 | 큰 파일을 읽는 작업인지 확인 |
| `wkB/s` | 초당 쓴 데이터 양 | 결과 파일이나 로그를 많이 쓰는지 확인 |
| `await` | I/O 요청 처리 평균 대기 시간 | 계속 높으면 디스크 응답이 느릴 수 있음 |
| `%util` | 디스크가 바쁘게 일한 비율 | 계속 높으면 디스크 병목 가능성 |

> `iostat`의 첫 번째 출력은 부팅 이후 평균값일 수 있다. 실시간 상태를 보려면 두 번째 출력부터 보는 것이 좋다.
