...
Docker는 `Dockerfile`을 바로 실행하는 것이 아니라, 먼저 이미지로 만든 뒤 그 이미지에서 컨테이너를 실행한다.

| 단계 | 설명 |
| --- | --- |
| Dockerfile | 어떤 운영체제 이미지에서 시작할지, 어떤 파일을 복사할지, 어떤 명령을 실행할지 적어둔 설계도 |
| Image | Dockerfile을 빌드해서 만든 실행 환경 묶음. 아직 실행 중인 프로세스는 아니다. |
| Container | 이미지를 실제로 실행한 상태. 컨테이너 안에서 스크립트나 프로그램이 실행된다. |
| Output | 컨테이너 안에서 만든 결과 파일은 볼륨 마운트를 통해 호스트 디렉터리에 남길 수 있다. |

바이오 분석 실습에서는 보통 아래 흐름으로 사용한다.

1. 분석 스크립트와 `Dockerfile`을 작성한다.
2. `Dockerfile`을 이미지로 빌드한다.
3. 입력 데이터 디렉터리와 출력 디렉터리를 컨테이너에 연결한다.
4. 컨테이너 안에서 분석 스크립트를 실행한다.
5. 작업이 끝나면 컨테이너는 종료되고, 결과 파일은 호스트의 출력 디렉터리에 남는다.

즉, Dockerfile은 실행 방법을 적은 파일이고, 이미지는 실행 환경의 저장본이며, 컨테이너는 그 이미지를 실제로 실행한 프로세스에 가깝다.
...
이 실습에서는 실제 유전체 데이터나 전문 바이오 분석 툴을 사용하지 않는다. 임의의 문자열이 많이 들어 있는 텍스트 파일을 가상의 분석 데이터로 보고, 컨테이너 안에서 `wc`, `awk`, `sort`, `uniq` 같은 리눅스 기본 명령어를 실행한다.
